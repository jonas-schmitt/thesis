...
These so-called \emph{boundary conditions} (BCs) can usually be classified in four different types:
\begin{description}
...
Usually, these points are defined on a grid or mesh with a particular structure.
...
One possibility to approximate a given PDE on a structured set of grid points is to compute the Taylor series expansion around each point, leading to the so-called \emph{finite difference method} (FDM).
