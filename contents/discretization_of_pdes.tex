...
\begin{equation}
	...
	\end{split}
\end{equation}
For the special case of $d = 3$, we can also provide an alternative definition based on the notation provided in Equation~\eqref{eq:3D-stencil-matrix-notation}.
\begin{equation}
	S^{(m, 3)} \bullet u_{x,y,z} = \sum_{i} S_{z}^{(m_{z+i},2)} \bullet u_{x,y,z+i}
\end{equation}
%TODO specify range of i
