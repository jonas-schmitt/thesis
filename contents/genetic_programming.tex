...
\end{equation*}
for a variable number of validation cases.
...
\begin{figure}
 ...
 \centering
		\includegraphics[scale=0.9]{figures/trees/gp_expression_tree1.pdf}
	\end{subfigure}
	\begin{subfigure}{0.41\textwidth}
  \centering
		\includegraphics[scale=0.9]{figures/trees/gp_expression_tree2.pdf}
	\end{subfigure}
...
\begin{figure}[h]
	\begin{subfigure}{0.58\textwidth}
 \centering
	\includegraphics[scale=0.46]{figures/trees/gp_derivation_tree1.pdf}
	\end{subfigure}
	\begin{subfigure}{0.41\textwidth}
 \centering
...
After an executable program is available in the target language, the final step within fitness evaluation is to measure its performance in a number of validation cases, which is then condensed to one or multiple measures of an individual's fitness.
For instance, one simple way to assess a program's quality in solving a number of validation cases is to represent its performance in each individual validation case as a separate optimization objective.
An alternative possibility is to represent different aspects of a program's quality in the form of distinct objectives, each of which is determined from a combination of its performance in multiple or even all validation cases.
...
Another recently proposed multi-objective selection operator that is especially geared towards program synthesis problems where the fitness evaluation is performed on multiple validation cases is lexicase selection.
In lexicase selection, individuals are selected according to their performance in a randomly chosen ordering of the validation cases, whereby decreasing precedence is given to each subsequent case.
