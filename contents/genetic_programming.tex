...
\begin{equation*}
	\begin{split}
		...
		A & \to -A \; | \; (A + A) \; | \; (A - A) \; | \; (A \cdot A) \; | \; x \; | \; y \\  
		B & \to \neg B \; | \; (B \wedge B) \; | \; (B \vee B) \; | \; u \; | \; v,
	\end{split}
\end{equation*}
where $T = \{S, E, A, B\}$ is the set of variables.
If we treat the arithmetic variables $x$, $y$ and the boolean variables $u$, $v$ as inputs, each expression generated by $G$ can be considered as a function $f(x,y,u,v)$.
By applying the standard rules of arithmetic and boolean algebra, we can then compute the result of each such function based on a given set of inputs.
