...
While expression trees can be both easily generated from a given program, as well as offering an efficient way to evaluate it, they possess an inherent limitation, which is the absence of type information.
...
\begin{figure}
	...
	\includegraphics[scale=0.525]{figures/trees/gp_derivation_tree1.pdf}
	\end{subfigure}
	\begin{subfigure}{0.39\textwidth}
	\includegraphics[scale=0.525]{figures/trees/gp_derivation_tree2.pdf}
	\end{subfigure}
	\caption{Derivation trees for $f_1$ and $f_2$.}
	\label{fig:gp-derivation-tree-examples}
\end{figure}
By investigating the structure of these trees, the advantage of this representation becomes obvious.
...
As both tree-based GGGP and GE both represent a trade-off between the simplicity of their genotype representation and high locality with respect to the phenotype, it is usually impossible to predict which variant will lead to a better outcome in practice.
...
\subsection{Fitness Evaluation and Elitism}
...
%In contrast, to the selection of the new population $P_i$ at the end of each generation, this phase usually allows certain individuals to be selected multiple times.
...
The most commonly used GP mutation operator is \emph{subtree mutation}, in which a certain branch of an individual is replaced by a randomly generated subtree, which can, for instance, be created using a similar tree-generation operator as within the population initialization~\cite{poli2008field}.
While subtree mutation has originally been defined for classical GP, it can be easily adapted to GGGP by introducing the additional constraint that branches with a certain variable as their root node can only be replaced by a subtree generated starting from the exact same variable. 
%TODO illustrate subtree mutation
\begin{figure}
	\centering
	\includegraphics[scale=0.4]{figures/trees/subtree_mutation.pdf}
	\caption{Illustration of subtree mutation on a derivation tree.}
	\label{fig:gp-subtree-mutation}
\end{figure}
Furthermore, a second mutation operator, which can be directly derived from subtree mutation, is \emph{insertion}.
While in subtree mutation the original branch is replaced and, hence, completely evicted from the original tree, insertion only creates a partial subtree at whose end the original subtree is then attached again.
In contrast to subtree mutation, whose applicability in the context of GGGP is independent of the grammar, insertion requires the generation of a subtree that includes the root node of the original branch as a child node at which the latter can then be attached.
While both subtree mutation and insertion, depending on the size of the randomly generated subtree, have the potential to drastically change the structure of the original tree, another possibility to alter a given tree is to replace only a single node with one of the same arity, which is called point mutation.
In the context of GGGP, this operation corresponds to the replacement of a production by an a different one that is available for the same variable.
