...
\begin{equation}
	\begin{split}
		...
		\ps A & \to -\ps A \; | \; (\ps A + \ps A) \; | \; (\ps A - \ps A) \; | \; (\ps A \cdot \ps A) \; | \; (\ps A / \ps A) \; | \ps{A}^{\ps{A}} \; | \; x \; | \; y \\  
...
%TODO describe how expression trees are obtained from derivation trees
...
\end{equation*}
based on which we can encode $f_1$ with the array of integers
\begin{equation*}
	\left[ 0, 0, 1, 0, 3, 4, 1, 3, 6, 7, 1, 4, 6, 7 \right].
\end{equation*}
Since we are always transforming the leftmost variable first, each number corresponds to a line in the derivation shown in Equation~\eqref{eq:gp-derivation-f1}.
While in this case, we have chosen each number from the range available for each production, in practice, we can always map an arbitrary positive integer to this range by applying a module $n$ operation, where $n$ is the number of available productions for each variable.
...
Therefore, even slight changes in the genotype of a GE representation can have a drastic effect on the phenotype expression.

As both tree-based GGGP and GE both represent a trade-off between the simplicity of their genotype representation and its closeness to the phenotype, it is usually impossible to predict which variant will lead to a better outcome in practice.
Since we have chosen to use tree-based GGGP in our implementation, we will exclusively focus on this variant in our treatment of mutation, crossover and fitness evaluation.
