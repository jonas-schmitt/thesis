...
The resulting state transition is implemented in the function \textsc{cocy} (coarse cycle).
\begin{algorithm}
	\begin{algorithmic}
...
The reason for this decision is that there is no benefit in performing a coarse-grid correction with the initial approximate solution $0$, and, thus, computing the residual always represents the first step of solving the coarse-grid error equation with subsequent multigrid operations.
