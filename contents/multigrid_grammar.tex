...
While Figure~\ref{fig:example-tree-grid-method-states} illustrates that the ternary tuple contains all relevant information of a multigrid methods current state on a certain level we have not yet considered how to transition between the states of two subsequent levels.
...
For instance in line 4 of Figure~\ref{fig:example-tree-grid-method-states} we have to extend the given ternary tuple $(0, \, I_{h}^{2h}(b_{h} - A_h x_{h}^0, \, \lambda)$ with $(x_{h}^0, \, b_h, \, \lambda, \, \lambda)$, such that all required information for restoring the previous fine-grid state values is included in the quaternary tuple $(0, \, I_{h}^{2h}(b_{h} - A_h x_{h}^0, \, \lambda, \, (x_{h}^0, \, b_h, \, \lambda, \, \lambda))$.
%TODO clarify that line 3 and 4 can in practice be combined to one step
Note that since $(x_{h}^0, \, b_h, \, \lambda, \, \lambda)$ represents the current state on the finest grid, its fourth component is empty, while otherwise it would refer to the previous state of the next level in the discretization hierarchy.
