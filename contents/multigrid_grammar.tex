...
\begin{itemize}
	...
	\end{equation*}
	\item \textbf{Cycle}: A new cycle is initiated by recursively solving the error equation on the next coarser level, whose right-hand side $b_{2h}$ is obtained by restricting the residual.
	\begin{align*}
		\tilde{x}_{2h} & = 0 \\
...
However, in contrast to the correction step which utilizes the current approximate solution on the coarse grid, both smoothing and the initiation of a new cycle on the coarse-grid require us to first compute the residual $r_h = b_h - A_h \tilde{x}_h$, which can be considered as an intermediate step.
