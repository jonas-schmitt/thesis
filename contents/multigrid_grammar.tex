...
\end{equation}
which corresponds to a polynomial of degree $k_{max}$.
...
%Now let us consider how different choices of $\nu_1$, $\nu_2$ and $\gamma$ lead to a different number of total smoothing steps for a five-grid method ($l = 4$).
%\begin{itemize}
%	\item $V(1,0)$: $\gamma = 1$, $\nu_1$ $\Rightarrow$  
%\end{itemize}
...
Furthermore, we restrict ourselves to those multigrid methods that employ at least one, but not more than six smoothing steps per level, whereby in each step we choose the smoother from a given number of alternatives.
First of all, we restrict ourselves to multigrid methods with a total number of smoothing steps 
\begin{equation}
	\left(l_{max} - l_{min}\right) \cdot \nu
\end{equation}
where $l_{min}$ is the minimum level, $l_{max}$ the maximum level, $\nu \in \left[1, 2, 3, 4, 5, 6 \right]$ and each smoother is chosen from a given number of alternatives $n$.
