...
To overcome the limitations of this formulation and construct multigrid methods with an arbitrary sequence of computations on each discretization level, a new formal language is needed.
The first step towards the development of such a language is to find a way to represent the current state within each step of a multigrid method.
Based on this representation, we can then define transition rules between the individual states, which will allow us to construct methods of arbitrary structure.
...
While Figure~\ref{fig:example-tree-grid-method-states} illustrates the ternary tuple $S_h$ contains all relevant information of a multigrid method's current state on a certain level, we have not yet discussed how we can transition between the states of two subsequent levels.
...
\begin{algorithm}
	\begin{algorithmic}
	\Function{cycle}{$A_{2h}$, $x_{2h}^0$, ($\tilde{x}_h$, $b_{h}$, $c_{2h}$, $S_{h/2}$)}
...
For the former, note that the initial problem corresponds the state $S_h^0 = (x_h^0, b_h, \lambda, \lambda)$.
...
Consequently, using the symbols $S$, $R$ and $P$ for smoothing, coarsening and coarse-grid correction\footnote{For the sake of simplicity, we use the starting letters of restriction and prolongation as abbreviations for coarsening and coarse-grid correction.}, respectively, in each step of the method, depending on the level, only the following sequences of operations can be defined:
\begin{align*}
...
Since each step within our simplified decision model includes the application of either $S_1$ or $S_2$ as a smoother which can then optionally be complemented with coarsening or coarse-grid correction, the depth of the tree is equal to the total number of smoothing steps $k$ applied within the corresponding multigrid method.
...
In the remainder of this thesis we will describe how G3P can be applied within \emph{EvoStencils}, our implementation of a grammar-guided optimization of multigrid methods in the Python programming language, which is based on the formal framework presented in this chapter.
