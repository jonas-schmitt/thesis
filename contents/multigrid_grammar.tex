...
As we have seen in Section~\ref{sec:multigrid-cycles}, according to the classical formulation of multigrid, each of these methods belongs to a particular family of cycles.
...
\begin{algorithm}[t]
	\begin{algorithmic}
		...
		\State ($x_h$, $b_{h}$, $\lambda$, $Z_{h/2}$) $\gets Z_{h}$
		\State $c_h \gets I_{2h}^{h} \cdot x_{2h}$
		\State return ($x_h$, $b_{h}$, $c_h$, $Z_{h/2}$)
		\EndFunction
	\end{algorithmic}
...
Furthermore, note that the evaluation of this sequence leads to a state whose first component $x_h$ contains an expression that corresponds to the stepwise execution of the method.
...
\section{A Novel Family of Multigrid Grammars}
...
\end{figure}
Each inner tree node corresponds to a symbol contained in the set of variables $V$ of our grammar, as shown in Equation~\eqref{eq:multigrid-grammar-variables}, while all leaf nodes refer to terminals.
To save space, the initial mandatory production that involves the start variable $\ps{S}$ is excluded.
Furthermore, since in the context of Algorithm~\ref{table:multigrid-grammar}, the application order of each function is well-defined, we have omitted all parentheses in the tree for the sake of simplicity.
...
%TODO rewrite this part, as it is a bit unclear
