...
As we have already mentioned, with the exception of Production~\eqref{prod:smoothing-operator},~\eqref{prod:partitioning} and~\eqref{prod:initial-solution} the right-hand side of each of the productions includes exactly one variable.
Furthermore, note that with the exception of the finest level, the number of available productions for the variables $\ps{s_H}$ and $\ps{c_H}$ on each level with a step size $H > h$ is always exactly two.
While the number of available choices for the variables $\ps{B_H}$, $\ps{P}$ and $\ps{\omega}$ are usually problem-dependent, the productions of $\ps{s_H}$ and $\ps{c_H}$ are both derived from the elementary multigrid operations formulated in Definition~\ref{def:elementary-multigrid-operations} and can, hence, be considered as a universal property of these methods.
As a consequence, if we neglect the choice of the smoother, partitioning and relaxation factor the number of different multigrid methods that can be generated by our grammar is of order $\mathcal{O}(2^N)$, where $N$ is the number of multigrid operations, i.e. smoothing, restriction and prolongation, employed within the method. 
