...
\end{enumerate}
With the formulation of this approach, we have now described all necessary steps from the grammar-based generation multigrid method to their transformation to an algorithmic representation based on which can then apply these methods as numerical solvers.
...
%TODO überleitung zu EvoStencils
...
In the remainder of this thesis we will present on the implementation of a GGGP-based approach for the optimization of multigrid methods in the Python programming language, called \emph{EvoStencils}, that is based on the formal framework presented in this section.
Furthermore, we will demonstrate the effectiveness of our approach by evolving efficient multigrid methods for a number of different partial differential equations (PDEs).
