...
According to Definition~\ref{def:elementary-multigrid-operations} $\tilde{x}_{h}$ can be either updated through smoothing or coarse-grid correction, which can both be expressed as a combination of state transition functions that leads to the productions
\begin{bnf}
	\bnfprod{$s_h$} {
...
\end{bnf}
In case of smoothing, the approximate solution is updated applying the operator $\ps{B_h}$ to a correction term that is included in the previous state represented by the variable $\ps{c_h}$.
...
\label{prod:residual}
\end{bnf}
Multiple smoothing steps can, therefore, be defined by means of repeated application of the productions~\eqref{prod:residual} and~\eqref{prod:smoothing}.
%TODO explain CGC
\begin{algorithm}
	\begin{bnf*}
		\bnfprod{$S$} {
