...
With the exception of the coarsest level, where the only allowed operation is the application of the coarse-grid solver
\begin{equation*}
	x_{H} = A^{-1}_H b_H,
\end{equation*}
where $H$ is the spacing of the grid points on this level, we can identify three elementary multigrid operations that can be performed within a cycle.
...
\begin{figure}
	\begin{equation*}
		\begin{array}{l l l}
			\hline
			\bm{Z_h} & \bm{=} & \bm{(\tilde{x}_h, \, b_h, \, r_h)}  \\
			1: & &  x_{h}^0, \, b_h, \, \lambda \\
			2: & &  x_{h}^0, \, b_h, \, b_{h} - A_h x_{h}^0 \\ \hline
			\bm{Z_{2h}} & \bm{=} &  \bm{(\tilde{x}_{2h}, \, b_{2h}, \, r_{2h})} \\
			3: & &  0, \, \lambda, \, \lambda \\
...
While Figure~\ref{fig:example-tree-grid-method-states} illustrates the ternary tuple $Z_h$ contains all relevant information of a multigrid method's current state on a certain level, we have not yet discussed how we can transition between the states of two subsequent levels.
...
\begin{equation*}
Z_h = (x_{h}^0, \, b_h, \, b_{h} - A_h x_{h}^0, \, \lambda) 
\end{equation*} 
...
\begin{itemize}
	...
	\item $Z_{h/2}$ is either the current state on the next finer level with a step size $h/2$ or the empty symbol $\lambda$ in case $h$ represents the topmost level in the given hierarchy of discretizations.
\end{itemize}
\end{definition}
Note that in Definition~\ref{def:multigrid-state}, the third component $c_{2h}$ of a multigrid state no longer has to refer to the residual but now represents an expression that corresponds to a general correction term.
...
For this purpose, first, the residual expression is assembled based on the system matrix $A_h$ and the given state $Z_h = (\tilde{x}_h, b_h, \lambda, Z_{h/2})$, which contains the current approximate solution $\tilde{x}_h$ and right-hand side $b_h$.
The resulting expression is included in $Z_h$ as a correction term $c_h$, which is returned at the end of the function.
\begin{algorithm}
	\begin{algorithmic}
...
\begin{algorithm}
	\begin{algorithmic}
		\Function{apply}{$B_h$, $(\tilde{x}_h, b_h, c_h, Z_h)$}
		\State return $(\tilde{x}_h, b_h, B_h\cdot c_h, Z_h)$
		\EndFunction
	\end{algorithmic}
...
%\caption{Iteration}
\end{algorithm}
In addition to the mere application of a correction term, this function also includes a relaxation factor $\omega$ and partitioning operator $P$, which enables the formulation of an underrelaxed, overrelaxed, and colored version of each operation, as described in Section~\ref{sec:rb-gs}.
...
Furthermore, note that while we have specified the pr\textbf{}olongation operator as a terminal symbol, it would as well be possible to specify its choice in the form of a separate production, as in the case of the smoothing operator $\ps{B_h}$.
...
For the former, note that the initial problem corresponds to the state $Z_h^0 = (x_h^0, b_h, \lambda, \lambda)$.
Since $Z_h^0$ represents the starting point of each possible sequence of multigrid operations, we have to include it as an additional production of the variable $\ps{s_h}$
\begin{bnf}
	\bnfprod{$s_h$} {
...
\begin{table}
	...
	\begin{bnf*}
		...
		\bnfmore {
			\bnfts{\textnormal{\textsc{update}}}(\bnfts{$\omega$}, \bnfsp \bnfes, \bnfsp \bnfts{\textnormal{\textsc{cgc}}}(\bnfts{$I_{2h}^h$}, \bnfsp \bnfpn{$s_{2h}$})) \bnfor (\bnfts{$x_h^0$}, \bnfsp \bnfts{$b_h$},\bnfsp \bnfes, \bnfsp \bnfes)
		} \\
		\bnfprod{$c_h$} {
			\bnfts{\textnormal{\textsc{residual}}}(\bnfts{$A_h$}, \bnfsp \bnfpn{$s_h$}) 
		} \\
...
\end{figure}
Again each node within this graph either corresponds to a predefined symbol or an operation defined on vectors and matrices, whereby in the latter case, there exists a direct edge to each node that refers to one of its arguments.  
