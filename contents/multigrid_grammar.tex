...
\begin{algorithm}
	...
	\end{algorithmic}
\end{algorithm}
First of all note, that a matrix-vector multiplication is always applied within an expression that corresponds to a correction term $c_h$, either within a smoother or in the form of a coarse-grid correction.
Therefore, based on our representation of a multigrid state, as formulated in Definition~\ref{def:multigrid-state}, we can define the following two functions that correspond to the respective state transitions
\begin{algorithm}
	\begin{algorithmic}
	...
	\end{algorithmic}
\end{algorithm}
In both cases, a new correction term is obtained by applying an operator ($B_h$ and $I_{2h}^h$) to a vector ($c_h$ and $\tilde{x}_{2h}$), which is then included into the state returned by both functions.
\begin{algorithm}
	\begin{algorithmic}
