...
To simplify the search space estimation, we, therefore, introduce the additional constraint that on each level the number of coarse-grid correction is always equal or lower than the number of smoothing steps.
Furthermore, we restrict ourselves to those multigrid methods that employ at least one, but not more than six smoothing steps per level.
Note that as a consequence the number of restriction and coarse-grid correction steps is, hence, also bound to be between one and six on each level.
For smoothing, we then always choose between two alternatives, for example Jacobi and red-black Gauss-Seidel, while we additionally assume that a restriction and coarse-grid correction step can only be performed immediately after smoothing, otherwise it is omitted. 
Now to arrive at a formula for the total number of multigrid variants that can constructed within this setting, we need to consider how many options are available within each individual step of the method.
Starting from the finest level, in each step of the method, we can decide whether we want to apply one of the two smoothers and if we subsequently want to proceed on the next coarser level using restriction, which means that we are granted with a total number of four options.
For the latter, we simply proceed at that level, where we are confronted with the same number of choices.
In case we decide to not perform any additional smoothing steps, the sequence on the current level is finished by either returning to the next finer level or in case of the finest level by ending the complete sequence.
%TODO arrive at formula for a particular number of smoothing steps : levels (except for the coarsest) * smoothing steps * 2 * 2 - levels (Option where not restriction is performed at all)
%TODO relaxation factors?


Finally, while we have already discussed that all derivations of our grammar are of the form of Equation~\eqref{eq:multigrid-grammar-derivation}, we have not yet considered how many different multigrid methods can be generated based on its productions.
First of all, note that with the exception of Production~\eqref{prod:smoothing-operator},~\eqref{prod:partitioning} and~\eqref{prod:initial-solution} the right-hand side of each of the productions includes exactly one variable.
Furthermore, with the exception of the finest level, the number of available productions for the variables $\ps{s_H}$ and $\ps{c_H}$ on each level with a step size $H > h$ is always exactly two.
While the number of available choices for the variables $\ps{B_H}$, $\ps{P}$ and $\ps{\omega}$ are usually problem-dependent, the productions of $\ps{s_H}$ and $\ps{c_H}$ are derived from the elementary multigrid operations formulated in Definition~\ref{def:elementary-multigrid-operations} and can, hence, be considered as a universal property of these methods.
As a consequence, if we neglect the choice of the smoother, partitioning and relaxation factor the number of different multigrid methods that can be generated by our grammar is of order $\mathcal{O}(\sum_{k = M}^N 2^k)$, where $M$ is the minimum and $N$ the maximum number of multigrid operations, i.e. smoothing, restriction and prolongation, employed within the method.
In contrast to this, in the traditional formulation of a multigrid cycle, as shown in Algorithm~\ref{alg:multigrid-cycle}, the computational structure of a multigrid method formulated on a certain hierarchy of discretizations is determined by the three parameters $\gamma$, $\nu_1$ and $\nu_2$.
If we assume that in practice usually either V-, F-, and W-cycles are employed and that the number of smoothing steps does not exceed a value of five, the number of structurally unique multigrid methods that can be described by means of these parameters is $3 \cdot 5 \cdot 5 = 75$.
On the other hand, if we for instance consider the grammar-based construction of those multigrid methods that employ at least ten, but not more than twenty operations on a given hierarchy of discretizations, the number of structurally unique methods is already greater than one million.
For comparison note that a five-grid V-cycle with $\nu_1 = 1$ and $\nu_2 = 0$, as shown in Figure~\ref{fig:five-grid-V-cycle}, employs a total number of four smoothing, three restriction and three prolongation steps and, hence, with the addition of the application of the coarse-grid solver on the coarsest level, the sum of multigrid operations is already equal to eleven. 
\begin{figure}
...
%TODO describe parameters for controlling the computational structure of a multigrid cycle
