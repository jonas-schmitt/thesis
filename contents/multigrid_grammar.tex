...
We can express this operation as a combination of the \textsc{update} and \textsc{cgc} transition functions, which leads to the production
\begin{bnf}
	\bnfprod{$s_h$} {
...
However, in contrast to smoothing the second argument of the \textsc{update} function, which represents the choice of a partitioning, is intentionally left empty, since it is not required in this case.
...
By utilizing a combination of the \textsc{apply} and \textsc{update} function, we can express this operation in form of the two production
\begin{bnf}
	\bnfprod{$c_{2H}$} {
