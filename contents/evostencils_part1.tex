...
Even though in all subsequent computations, the first entry of this tuple consists of a \texttt{Cycle} object, for the sake of simplicity, we simply include the right-hand side already included in this object as a second tuple entry, which enables us to utilize the same \texttt{residual} function within all grammar productions.
For this purpose, we need to adapt the \texttt{iterate} function accordingly, such that the respective binary state tuple, in the form of a \texttt{Cycle} object, which computes a new approximate solution, and the current right-hand side, is returned.
Finally, as the last argument of the\texttt{coarse\_grid\_correction} function result of an application of the \texttt{iterate} function, it needs to be treated in a similar way.

The second main difference in the implementation shown in Listing~\ref{code:grammar:basic-functions} compared to our original definition of the \textsc{iterate} function is the representation of the relaxation factor as an index within a uniformly-sampled interval, which is included in the respective \texttt{Terminal} object.
...
In Definition~\
%TODO continue here
%TODO maybe rename iterate to update
