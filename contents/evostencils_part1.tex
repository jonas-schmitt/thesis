...
Finally, using the functions \emph{init\_grammar} and \emph{add\_level}, as defined in Listing~\ref{code:grammar:init-grammar} and~\ref{code:grammar:add-level}, respectively, we can implement a Python function \emph{generate\_grammar} for generating the primitive set of arbitrary multigrid grammars, which is shown in Listing~\ref{code:grammar:generate-grammar}.
\begin{listing}
...
In order to provide an accessible interface to EvoStencils' functionality, we implement all steps from the construction of a grammar based on the given PDE-based problem to the execution of the actual optimization procedure within an \emph{Optimizer} class, whose basic implementation is shown in Listing~\ref{code:optimization:optimizer}.
\begin{listing}
	\inputminted[linenos]{python}{evostencils/optimization/optimizer.py}
	\caption{Optimizer class}
	\label{code:optimization:optimizer}
\end{listing}
In addition to the generated primitive set, this class relies on two main components, a \emph{Toolbox} object for the execution of the GGGP-based evolutionary search procedure, and a \emph{ProgramGenerator} object, which enables the generation and evaluation of solver implementations while also providing an interface to all required information about the given PDE-based problem, for instance, in form of the respective operators.
Based on the functionality of these two components, which, in the following, will be described in detail, we can implement a \emph{run} method, which first generates a \emph{PrimitiveSetTyped} and then, based on the initialized \emph{Toolbox} object, performs a grammar-guided evolutionary search in order to find a population of individuals that represent efficient multigrid methods for the given problem.
Note that within this search procedure, the evaluation of each generated solver relies on an \emph{evaluate} method, which returns suitable metrics for assessing its quality.

\section{Evolutionary Search Method}
After translating our class of multigrid grammars introduced in Section~\ref{sec:multigrid-grammar} into a strongly-typed GP system, we can now utilize the functionality of DEAP's GP module to implement the evolutionary search operators introduced in Section~\ref{sec:gggp}.
...
The DEAP framework implements random tree generation in form of the \emph{generate} function, which constructs a tree based on a given primitive set while enforcing that the length of each of its branches satisfies a certain depth limit.
...
To implement subtree replacement and insertion within a single function, we, again, utilize our custom \emph{generate} function, which already provides the necessary functionality.
...
Note that in contrast to $n_{\varepsilon}$, the execution time $t$ solely depends the computational structure of the employed iterative method and is, hence, independent of $\varepsilon$.
...
However, as we have discussed in Section~\ref{sec:multigrid-cycles}, the choice of each method usually represents a compromise between a low number of computations and, thus, a fast execution time and a fast convergence, which usually requires more computations.
While we have now discussed how to assess the quality of an iterative method, the question remains how we should choose one or multiple optimization objectives based on these metrics.
Here the main challenge is to achieve \emph{generalization}, which means that our goal is to find multigrid methods that are robust in solving problems with similar characteristics.
Theoretically, we could simply execute a solver on each individual problem instance in order to measure its solving time.
However, due to the high computational demands of solving PDEs in many domains, this approach is not feasible in practice.
...
Even though, in principle, one could imagine deriving a unified model for estimating the quality of a multigrid method, due to the entirely different nature of the two performance-guiding metrics $t$ and $\rho$, we consider them separately.
First of all, note that the execution time $t$ of each step of an iterative method entirely depends on the amount of computations and their execution efficiency on a modern compute platform.
In contrast, the convergence factor $\rho$ is directly linked to the mathematical properties of the solver, for instance, its capability to efficiently reduce certain error components in each iteration, as we have discussed in Section~\ref{sec:multigrid-methods}. 
While the availability of performance models for modern computer architectures, as the roofline~\cite{williams2009roofline} and execution-cache memory model~\cite{hager2016exploring}, enable predictions in an automated and deterministic manner, analyzing the mathematical properties of a solver in a reliable manner is often difficult.
Local Fourier analysis (LFA)~\cite{wienands2004practical} represents a promising alternative for analyzing the mathematical properties of a solver in a resolution-independent manner.
Even though LFA has already been applied successfully to numerous applications~\cite{rodrigo2017validity}, only recently freely-available software tools have become available that automate its execution~\cite{rittich2018extending,kahl2020automated}.
To test the applicability of the library LFA Lab~\footnote{LFA Lab: \url{https://github.com/hrittich/lfa-lab}} for the automated convergence estimation of grammar-generated multigrid methods, we have already performed a number of experiments in~\cite{schmitt2020constructing,hoefer2020comparing}.
...
A second option to lower evaluation cost is to estimate the quality of a given solver by measuring its characteristics on a number of \emph{proxy} problems, that are faster to solve than the original problem but which still possess similar mathematical properties.
...
Hence, if a given multigrid method is able to achieve $h$-independent convergence on a sequence of increasingly finer resolved instances of the same PDE, there is a high probability that the method also generalizes to other problem instances.
However, the application of this approach requires us to generate efficient implementations for each multigrid method considered, which can then be executed to obtain the relevant quality metrics for each proxy problem.
...
Recently, code generation techniques, that enable us to achieve this goal, have become available~\cite{kostler2020code,schmitt2018automating}.
...
It enables the formulation of multigrid methods in a level-independent domain specific language (DSL), called ExaSlang~\cite{schmitt2014exaslang,schmitt2016systems}, which is almost indistinguishable from the textbook-like description of an algorithm.
...
\end{listing}
As this example demonstrates ExaSlang enables the formulation of the same operations on different levels of a discretization hierarchy based on so-balled \emph{level declarations}\footnote{In case no level declaration is provided, ExaStencils assumes that the operation is applied on the current level.}.
Similar to the specification of the grid spacing as a subscript in Algorithm~\ref{alg:example-three-grid-method}, we can utilize this notation to formulate each operation within a multigrid method relative to the \emph{finest} level.
...
Therefore, ExaStencils is ideally-suited for the automatic generation and evaluations of multigrid methods within our evolutionary optimization.
While ExaStencils is implemented in the Scala programming language and can, thus, not be accessed directly within Python code it provides simply configuration files, which allow us to adapt the certain problem characteristics, such as the grid spacing and number of coarsening steps.
In addition, certain code generation options, such as compiler-based performance optimizations, can be en- or disabled.
To encapsulate the usage of the ExaStencils compiler, EvoStencils provides the \emph{ProgramGenerator} class.
In addition to the automatic adaption of the respective configuration files and the actual execution of the code generation process, this class also includes functionality for extracting all information about a given problem based on an existing ExaSlang specification. 
Therefore, in case this specification contains all necessary informations for the generation of the respective multigrid grammar, this allows us integrate EvoStencils directly into ExaStencils' solver generation workflow.
It, furthermore, enables the application of our optimization approach to many of the problems that are already available within ExaStencils.
Based on the functionality provided by the \emph{ProgramGenerator} class, we can then implement an evaluation function in form of the \emph{evaluate} method of previously mentioned \emph{Optimizer} class, as shown in Listing~\ref{code:optimization:evaluate}.
\begin{listing}
	\inputminted[fontsize=\footnotesize,breaklines]{scala}{evostencils/optimization/evaluate.py}
	\caption{}
	\label{code:optimization:evaluate}
\end{listing}
This method then provides a high-level interface to the evaluation of an arbitrary individual that has been generated based on a given \emph{PrimitiveSetTyped} object.
...
Therefore, we first check whether the given individual has been already evaluated before, in which case we simply return the cached value.
Otherwise, we utilize the \emph{generate\_and\_evaluate} method of the \emph{ProgramGenerator} class to generate a C++ implementation for the corresponding solver, using the ExaStencils compiler, which is then executed on the current proxy problem.
...
Furthermore, to compute the solver's execution time per iteration $t$, we simply divide the total solving time by the number of iterations.
...
However, it is also possible to implement a similar function that returns a single-objective fitness value, for instance, in form of the total solving time.
...
After successfully obtaining a \emph{Fitness} object for each individual, we can complete our collection of evolutionary operators with the definition of suitable selection and elitism operations.
As we have discussed in Section~\ref{sec:gggp-evaluation-and-selection}, in contrast to initialization, mutation and crossover, the selection of individuals within an evolutionary algorithm is independent of their underlying representation, but solely depends on their fitness attribute.
...
Listing~\ref{code:optimization:selection} shows the example initialization of a \emph{Toolbox} object using the popular NSGA-II selection operator~\cite{deb2002fast}. 
\begin{listing}
...
In accordance with the common notation to describe evolutionary algorithms~\cite{back1997handbook} based on two parameters $\mu$ and $\lambda$, this function implements a $\left(\mu + \lambda \right)$ strategy.
Therefore, in each generation $\lambda$ new individuals are created based on $\mu$ parent individuals using either mutation and crossover, while the new population is then selected from the combined set of parent and child individuals using the defined elitism operator. 
In order to initiate the search with a sufficiently diverse population, the number of initially-generated individuals can be set higher than $\mu$, which is controlled by the parameter \emph{initial\_population\_size}.
In Line 11, $\mu$ individuals are then selected for the first generation, using the registered \emph{elitism} operator.
The actual search is then performed for a predefined number of generations.
As a fallback solution, we additionally implement a simple random search, whereby in each generation we randomly generate $\lambda$ individuals, which can be seen in Line 15 of Listing~\ref{code:optimization:evolutionary_search}.
In contrast, within the evolutionary algorithm, we first select $\lambda$ individuals for crossover and mutation, using the registered \emph{select} operator, as shown in Line 18.
As mutation represents a modifying operation, we first generate an identical copy of each selected parent individual, based on which then either crossover or mutation is applied, depending on a predefined probability.
Finally, in Line 34--40 the resulting newly created individuals are evaluated and a new elitist population of size $\mu$ is obtained from the combined set of parent and child individuals.
...
Furthermore, in order to accelerate the evaluation of the large number of individuals required to obtain competitive solvers for many PDEs, we will demonstrate how our implementation can be parallelized on multi-node systems using the message passing interface (MPI).   
