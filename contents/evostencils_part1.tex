...
If we assume the availability of such a function, the next step is then implement the actual application of a smoother, in form of the function \emph{smoothing}, shown in Listing~\ref{code:grammar:smoothing}.
...
In Production~\ref{prod:coarse-coarse-grid-correction} we then additionally apply the \textsc{update} function to generate an expression that updates the current approximate solution with this term.
...
For the sake of consistency, we have separated the construction of the coarse problem from the actual correction step.
However, as the application of a solver is the only allowed operation on the coarsest level, this separation is not strictly necessary from a practical point of view and we can implement the complete process in a single function \emph{correct\_with\_coarse\_grid\_solver}, whose implementation is shown in Listing~\ref{code:grammar:coarse-grid-solver}.
Similar to the \emph{initiate\_cycle} function, we first restrict the correction term of a given \texttt{Cycle} object.
