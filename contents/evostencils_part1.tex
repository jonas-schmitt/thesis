...
Note that the coarse-grid solver can additionally contain an expression, which then again represents a multigrid method, given in form of the intermediate representation introduced in the last section.
Based on these terminals we can next implement the state transition functions shown in Table~\ref{table:grammar-semantics}.
However, at this point it is important to keep in mind, that our intermediate representation deviates from the expressions generated within the functions shown in Table~\ref{table:grammar-semantics} with respect to its treatment of multiply occurring subexpressions, in particular in form of the previous approximate solution and right-hand side on each level.
While the construction of a computational graph similar to Figure~\ref{fig:example-three-grid-method-computational-graph} based on the expression of a multigrid method, obtained from a grammar derivation tree, can be considered as an additional step, from an implementation perspective, we can simplify this process to a single operation.