...
\subsection{Translation to Strongly-Typed Genetic Programming}
...
Next, a new \emph{PrimitiveSetTyped} object is created, in which we set the return type to $\ps{E}$, similar to the choice of the start variable within the grammar.
We then proceed to define each production in the form of a \emph{Primitive} objective, which consists of a function, a list of input types, and an output type.
...
While the example considered here does not require significant adjustments to the structure of the grammar as the type \emph{E} can already be converted to \emph{A} using the identity function, in general, this is not the case.
...
While in Listing~\ref{code:grammar:pset-example}, we have encoded each variable by an actual Python type, at this point, we have to introduce an additional constraint that prevents us from pursuing the same approach.
