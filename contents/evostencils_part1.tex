...
After adding each of the grammar's productions to the set in form of a \emph{Primitive} object, what remains is the treatment of the four terminals $x$, $y$, $u$ and $v$.
In DEAP terminals are represented by objects of the \emph{Terminal} class, which corresponds to a \emph{Primitive} without any arguments, and, thus, an empty list of input types.
...
Since, similar to Section~\ref{sec:gggp-representation}, we assume that each of the four terminals represents an argument of the generated Python function, we additionally append it to the list of arguments, which is shown in line~32 of Listing~\ref{code:grammar:pset-example}.
Note that for each of the two symbols $x$ and $y$ we include two \emph{Terminal} objects into the set, which only differ in their return type.
...
Finally, after generating a \emph{PrimitiveSetTyped} object that corresponds to our example grammar, we can generate a random tree using the \emph{genGrow} function, which corresponds to aforementioned \emph{grow} initialization operator.
Based on the resulting tree, a function object can then be generated using DEAP's \emph{compile} function, which can then be executed similar to any other Python function by providing a value for each of its arguments.
