...
As a remedy, we can try to estimate the quality of a given solver by measuring its characteristics on a number of \emph{proxy} problems.
A second possibility is the application of \emph{predictive models} to obtain an estimation without the need to actual execute the solver on any problem.
...
First of all, note that the execution time $t$ of each step of an iterative method entirely depends on the amount of performed computations and their execution efficiency on a modern compute platform.
...
To test the applicability of these tools, called LFA Lab~\footnote{LFA Lab: \url{https://github.com/hrittich/lfa-lab}} for the automated convergence estimation of grammar-generated multigrid methods, we have already performed a number of experiments in~\cite{schmitt2020constructing}.
...
%TODO include Daniel Höfer master thesis ref
 
 

%TODO derive formula for number of iterations based on \rho and \varepsilon!
\begin{listing}
