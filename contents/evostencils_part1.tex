...
In order to provide an accessible interface to EvoStencils' functionality, we implement all steps from the construction of a grammar based on the given PDE-based problem to the execution of the actual optimization procedure within the \emph{ProgramOptimizer} class.
...
ExaStencils~\cite{lengauer2014exastencils,lengauer2020exastencils} is a code generation framework, implemented in the Scala programming language, that has been specifically designed for the automatic generation of scalable multigrid solver implementations on modern parallel computing hardware.
...
\begin{listing}
	\inputminted[fontsize=\footnotesize,breaklines]{scala}{evostencils/three_grid_example.exa3}
