...
After successfully obtaining a \emph{Fitness} object for each individual, we complete our toolbox of evolutionary operators with the definition of suitable selection and elitism operators.
...
The resulting implementation of the \emph{evolutionary\_search} method is shown in Listing~\ref{code:optimization:evolutionary_search}. 
\begin{listing}
...
In accordance with the common notation to describe evolutionary algorithms based on two parameters $\mu$ and $\lambda$, this function implements a $\left(\mu + \lambda \right)$ strategy.
