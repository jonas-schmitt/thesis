...
Within this data structure, a type attribute can be initialized in two different ways depending on whether the types of the next-higher level in the discretization hierarchy are provided.
...
Based on this function, we then generate a \emph{Type} object with unique \emph{identifier} for each variable of two subsequent levels of the discretization hierarchy, whereby each attribute with a subscript $h$ and $2h$ corresponds to a variable on the current level and next-coarser level, respectively.
...
Finally, in addition to the types that need to be defined for each level, there exist a number of level-independent variables, for which only a single type needs to be defined for the whole grammar.
While we have already discussed the purpose of the \emph{Partitioning} and \emph{RelaxationFactorIndex} attributes in Section~\ref{sec:evostencils:state-transition-functions}, we additionally create a \emph{Type} object for representing the shapes of rectangular blocks within the definition of block smoothers, which have been briefly discussed in Section~\ref{subsec:block-smoothing}.
