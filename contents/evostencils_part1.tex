...
First of all, Listing~\ref{code:grammar:basic-functions} contains the implementation of the functions \texttt{residual}, \texttt{apply} and \texttt{iterate}, which correspond to the eponymous state transitions functions defined in Table~\ref{table:grammar-semantics}.
\begin{listing}
...
The second main difference in the implementation shown in Listing~\ref{code:grammar:basic-functions} compared to our original definition of the iterate function, which is the representation of the relaxation factor as an index within interval included in the respective \texttt{Terminal} object.
...
Next we consider the implementation of the functions \texttt{coarse\_cycle} and \texttt{coarse\_grid\_correction} shown in Listing~\ref{code:grammar:basic-functions}, which are responsible for transferring information between adjacent grids within the hierarchy of discretizations.
Again our implementation of these functions directly correspond to the respective state transition functions \textsc{cocy} and \textsc{cgc} defined in Table~\ref{table:grammar-semantics}.
While these four functions provide us the basic functionality to generate the IR representation of arbitrarily structured multigrid method on each level, we have not yet considered how we can apply different smoothers, restriction and the coarse-grid solver.
...
\end{listing}
Finally, both the creation of a new cycle on the next coarser level as well as the application of the coarse grid solver on the coarsest level require us to restrict the correction part of a given \texttt{Cycle} object.
While this can be accomplished within a single application of the \texttt{apply} function, for the sake of clarity, we still implement this functionality in a separate function, which is shown in Listing~\ref{code:grammar:restriction}.
\begin{listing}
	\inputminted{python}{evostencils/grammar/restrict.py}
	\caption{Restriction function.}
	\label{code:grammar:restriction}
\end{listing}
Note that in Table~\ref{table:multigrid-grammar} the application of this function corresponds to the 
%TODO adapt according to updated functions in current evostencils code
