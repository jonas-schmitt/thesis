...
Note that as the \texttt{Approximation} and \texttt{RightHandSide} class, from a mathematical point of view, can be both considered as a vector and we simply derive them from each other.
As a consequence we can still differentiate the objects of both classes with respect to their type, while avoiding unnecessary code duplication.
In addition to the attributes defined in its parent class, the \texttt{Approximation} 
