...
For this purpose, we need to adapt the \emph{iterate} function accordingly, such that the respective binary state tuple, in the form of a \emph{Cycle} object, which computes a new approximate solution, and the current right-hand side, is returned.
Finally, as the last argument of the\emph{coarse\_grid\_correction} function result of an application of the \emph{iterate} function, it needs to be treated in a similar way.

The second main difference in the implementation shown in Listing~\ref{code:grammar:basic-functions} compared to our original definition of the \textsc{iterate} function is the representation of the relaxation factor as an index within a uniformly-sampled interval, which is included in the respective \emph{Terminal} object.
...
For instance, smoothing is performed through consecutive application of \textsc{apply} and \textsc{iterate}.
...
If we assume the availability of such a function, the next step is then implement the actual application of a smoother, in form of the function \emph{smoothing\_iteration}, shown in Listing~\ref{code:grammar:smoothing}.
Similar to Production~\ref{prod:smoothing}, the implementation of this function consists of a combination of \textsc{apply} and \textsc{iterate}, whereby we generate $\ps{B_{h}}$ using the aforementioned function.
...
Similarly, the second basic multigrid operation \emph{coarsening} is realized by combining two basic state transition functions: \textsc{apply} and \textsc{cycle}.
While the former applies a restriction operator to the previously generated residual expression, the latter is then utilized to initiate a new cycle on the next coarser level using the restricted residual as a right-hand side.
Finally, note that the coarse-grid correction operation in Definition~\ref{def:elementary-multigrid-operations} differs from the identically named state transition function in the respect that it additionally applies the computed correction to the current approximate solution.
...
In Production~\ref{prod:coarse-coarse-grid-correction} we then additionally apply the \textsc{iterate} function to generate an expression that utilizes this term to correct the previous approximate solution in each iteration of the method.
Listing~\ref{code:grammar:inter-grid-operations} shows the implementation of Production~\ref{prod:coarsening} and~\ref{prod:coarse-grid-correction} in form of the functions \textsl{coarsening} and \emph{coarse\_grid\_correction\_iteration}.
In case of the latter we have included the suffix \emph{iteration} to make it distinguishable from the respective state transition function.
Finally, we need to provide an implementation for the application of a solver on the coarsest level, which is represented by the Productions~\ref{prod:coarse-grid-solver} and~\ref{prod:coarse-grid-solver-correction}.
While, for the sake of consistency, we have separated the construction of the coarse problem from the actual correction step, as the application of a solver is the only allowed operation on the coarsest level, this separation is not strictly necessary from a practical point of view.
Instead, we can implement the complete process in a single function that first restricts the correction term of a given \texttt{Cycle} object, to which then the actual coarse-grid solver is applied.
Finally, similar to the coarse-grid correction on other levels, the obtained solution, which is supposed to be exact, is then prolongated and then utilized to correct the previous approximate solution on the respective level.
%TODO Ref to function restrict_solve_prolongate, explain splitting into two functions
\begin{listing}
