...
\end{listing}
As their names suggest these function correspond to the state transition functions \textsc{cocy} and \textsc{cgc} defined in Table~\ref{table:grammar-semantics}.
While the \texttt{coarse\_cycle} function corresponds directly to its eponymous counterpart, the \texttt{coarse\_grid\_correction}, for the sake of simplicity, additionally applies the \texttt{iterate} function at the end of its computation.
The latter is justified by the fact that in our multigrid grammar, as shown in Table~\ref{table:multigrid-grammar}, the \textsc{cgc} function is always applied in conjunction with the \textsc{iterate} function.
%TODO continue here: After defining the basic 
