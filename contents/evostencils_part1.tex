...
In order to provide an accessible interface to EvoStencils' functionality, we implement all steps from the construction of a grammar based on the given PDE-based problem to the execution of the actual optimization procedure within an \emph{Optimizer} class.
Internally, this class stores the generated \emph{PrimitiveSetTyped} together with all necessary information about the discretized PDE that the generated solvers aim to solve. 
...
Based on the functionality provided by the \emph{ProgramOptimizer} class, we can then implement an evaluation function in form of the \emph{evaluate} method of previously mentioned \emph{Optimizer} class, as shown in Listing~\ref{code:optimization:evaluate}.
\begin{listing}
