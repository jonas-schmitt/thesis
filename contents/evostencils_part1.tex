...
\begin{listing}
	\inputminted[linenos,numbersep=5pt,numberblanklines=False]{python}{evostencils/grammar/pset_example.py}
	\caption{Grammar Generation with DEAP}
	\label{code:grammar:pset-example}
\end{listing}
Here, the first step is to create a unique type for each symbol that is contained in the set of variables $V = \left\{\ps{E}, \ps{A}, \ps{B} \right\}$, which can be accomplished using Python's builtin \emph{type} function as shown in line~5--7 of Listing~\ref{code:grammar:pset-example}.
Next, a new \emph{PrimitiveSetTyped} object is created in line~8, in which we set the return type to $\ps{E}$, similar to the choice of the start variable within the grammar.
...
\begin{listing}
  	\inputminted[linenos,numbersep=5pt,numberblanklines=False]{python}{evostencils/grammar/types.py}
...
\begin{listing}
	\inputminted[linenos,numbersep=5pt,numberblanklines=False]{python}{evostencils/grammar/init_grammar.py}
...
Based on these data structures, we proceed with the creation of the \emph{PrimitiveSetTyped} object in line~16.
We then include the initial state as a tuple consisting of the respective \emph{Approximation} and \emph{RightHandSide} object and all level-independent terminals in line~17--26.
...
All other \emph{Terminal} and \emph{Primitive} objects are then included in line~28 using the \emph{add\_level} function shown in Listing~\ref{code:grammar:add-level}.
...
For this purpose, we set the output type of the respective \emph{Terminal} object in line~17 of Listing~\ref{code:grammar:init-grammar} to the \emph{S\_guard\_h} attribute of the respective \emph{Types} data structure.
...
\begin{listing}
	\inputminted[linenos,numbersep=5pt,numberblanklines=False]{python}{evostencils/grammar/add_level.py}
...
For each of the \emph{Primitive} objects generated in line~9--14 whose list of input types contains only unguarded types, the output type is also unguarded.
...
We can, thus, enforce that the coarse-grid solver is applied at least once within each derivation by including the necessary transition in line~17.
...
We, therefore, have to check whether we have arrived at the coarsest grid of the discretization hierarchy, which is done in line~12 of Listing~\ref{code:grammar:add-level}.
...
\begin{listing}
	\inputminted[linenos,numbersep=5pt,numberblanklines=False]{python}{evostencils/grammar/generate_grammar.py}
...
We first create an initial \emph{PrimitiveSetTyped} to which we add all required \emph{Terminal} and \emph{Primitive} objects on the finest grid using the previously defined function \emph{init\_grammar} in line~9.
...
For this purpose, we only have to create the respective \emph{Terminals} and \emph{Types} data structures, as shown in lines~22 and~24, which are then passed to \emph{add\_level} in line~25 to generate all required \emph{Primitive} and \emph{Terminal} objects.
...
\begin{listing}
	\inputminted[linenos,numbersep=5pt,numberblanklines=False,stepnumber=13]{python}{evostencils/optimization/optimizer.py}
...
Based on the functionality of these two components, we can define the \emph{run} method of the \emph{Optimizer} class, whose implementation is shown in line~23--34 of Listing~\ref{code:optimization:optimizer}.
...
\begin{listing}
	\inputminted[linenos,numbersep=5pt,numberblanklines=False]{python}{evostencils/gp/toolbox.py}
...
To facilitate the implementation of both the \emph{Fitness} and \emph{Individual} class, in line~6 -- 10, we make use of DEAP's \emph{creator} module.
...
Based on the definition of the \emph{Fitness} and \emph{Individual} class, we can then create the actual \emph{Toolbox} object together with the evolutionary operators.
...
Note that in contrast to $n_{\varepsilon}$, the execution time $t$ solely depends on the amount of computation performed within each iteration and is thus independent of $\varepsilon$.
...
While we have now derived suitable metrics for assessing the quality of an iterative method, we have not yet answered how to choose one or multiple optimization objectives based on this information.
...
A second option to decrease the evaluation cost is to estimate the quality of a given solver by measuring its characteristics on a number of proxy problems that are faster to solve than the original problem while possessing similar mathematical properties.
...
% \begin{lstlisting}[language=ExaSlang3]
...
\begin{listing}
	\inputminted[linenos,numbersep=5pt,numberblanklines=False]{python}{evostencils/optimization/selection.py}
...
\begin{listing}
	\inputminted[linenos,numbersep=5pt,numberblanklines=False]{python}{evostencils/optimization/evolutionary_search.py}
