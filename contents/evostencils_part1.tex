...
In line~16--22, we then include the initial state as a tuple consisting of the respective \emph{Approximation} and \emph{RightHandSide} object and a number of level-independent terminals.
...
All other \emph{Terminal} and \emph{Primitive} objects are then included in line~24 using the \emph{add\_level} function shown in Listing~\ref{code:grammar:add-level}.
...
In We first construct an initial \emph{PrimitiveSetTyped} to which we add all required \emph{Terminal} and \emph{Primitive} objects on the finest grid using the previously defined function \emph{init\_grammar}.
