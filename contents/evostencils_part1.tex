...
Therefore, the first step within our implementation of an evolutionary search method for the grammar-guided optimization of multigrid methods is the randomized creation of an initial population.
As we have already discussed in Section~\ref{sec:gggp-initialization}, the \emph{full} initialization operator~\cite{koza1994genetic,poli2008field} is only applicable in cases, where the grammar enable the termination of unfinished branches at any given point in the derivation through, by invoking a terminal production.
...
The DEAP framework provides an implementation of this operator in form of the \emph{genGrow} function, which generates a random tree based on a given primitive set while enforcing that the length of each of its branches satisfies a certain depth limit, chosen randomly from an interval defined by the parameters \emph{min\_height} and \emph{max\_height}.
...
To resolve this issue, we, therefore, need to adapt DEAP's implementation of the \emph{genGrow} function, such that the specified limit is only applied whenever possible.
...
The resulting adapted implementation of the \emph{genGrow} function is shown in Section~\ref{appendix:gp} of the appendix.
\subsection{Genetic Operators}
Based on our implementation of the \emph{grow} operator and DEAP's builtin functionality, we can now define all remaining components of a  GGGP-based search method.
