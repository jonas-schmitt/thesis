...
\begin{equation}
	\begin{split}
		\frac{\partial^2}{\partial x^2} u(x,y) + \frac{\partial^2}{\partial y^2} u(x,y) & = f(x, y) \quad \text{in} \; \left( 0, 1 \right)^2 \\
		u(x,y) & = 0 \quad \text{on} \; \left( 0, 1 \right)^2 
	\end{split}
	\label{eq:2D-poisson-model}
\end{equation}
Note that this equation corresponds to a two-dimensional steady-state heat equation with constant $\alpha = 1$.
We then discretize Equation~\eqref{eq:2D-poisson-model} using finite differences and a uniform step size $h$, which yields
\begin{equation}
	\begin{split}
		\frac{1}{h^2} (u_{i-1, j} + u_{i+1, j} + u_{i, j-1} + u_{i, j+1} - 4 u_{i,j} ) & = f_{i, j} \quad \text{in} \; \left( 0, 1 \right)^2 \\
		u_{i, j} & = 0 \quad \text{on} \; \left( 0, 1 \right)^2 
	\end{split} 
	\label{eq:2D-poisson-model-discrete}
\end{equation}
with $u_{i,j} = u(ih, jh)$ and $f_{i,j} = f(ih, jh)$ for $i = 1, 2, \dots, n$ and $j = 1, 2, \dots, n$, where $n = 1/h - 1$.
By defining a unique ordering of the grid points $u_{i, j}$, Equation~\eqref{eq:2D-poisson-model-discrete} we can be transformed to a system of linear equation of type $\bm{A} \bm{u} = \bm{f}$. 
