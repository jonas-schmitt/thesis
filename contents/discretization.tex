...
\begin{equation}
	...
	\\ & \bm{a_k} \in \mathbb{Z}^n \wedge \bm{a_i} \neq \bm{a_j} \; \forall \, i, j \in \left\{1, 2, \dots m \right\} \; \text{with} \; i \neq j , b_k \in \mathbb{C}.
	\end{split}
\end{equation}
...
% Diagonal
\begin{equation}
		\text{diag}\left(S^{(m, n)}\right) = \begin{cases}
			\left\{\left(\bm{0}, b\right) \right\} & \text{if} \; (\bm 0, b) \in S^{(m, n)} \\
			\emptyset & \text{else}
		\end{cases}
\end{equation}
\begin{equation}
	S^{(m, n)} + \tilde{S}^{(\tilde{m}, \tilde{n})} = \left\{\left(\bm{a_k}, b_k + \tilde{b} \right) \right\} \; \forall 
\end{equation}
