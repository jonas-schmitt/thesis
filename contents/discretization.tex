...
\begin{equation}
	\begin{split}
			& S^{(m, n)} = \{(\bm{a_k}, b_k) \}_{k=1}^m = \{(\bm{a_1}, b_1),  (\bm{a_2}, b_2), \dots (\bm{a_m}, b_m)\} 
...
\begin{equation}
	\begin{split}
		...
		\text{with} \; \bm{x} \in \mathcal G \supset \mathbb{N}^n, \\ & (\bm{a_i}, b_i) \in S^{(m,n)} \; \forall \, i \in \{ 1, 2, \dots m \} 
	\end{split}
\end{equation}
...
\begin{equation}
	\Delta_h^{(5,2)} \circ u_{i,j} = \frac{1}{h^2} \left(4 u_{i+0,j+0}  - u_{i-1,j+0} - u_{i+1,j+0} - u_{i+0,j-1} - u_{i+0,j-1}\right),
\end{equation}
...
\begin{equation}
	\alpha S^{(m, n)} = \{(\bm{a_k}, \alpha b_k) \}_{k=1}^m
\end{equation}
% Diagonal
\begin{equation}
...
\begin{equation}
	S_1 \pm S_2 = 
	\begin{cases}
		\{(\bm{a}, b\pm c ) \} \cup (\tilde{S}_1 \pm \tilde{S}_2) & \text{if} \; S_1 = 	\{(\bm{a}, b ) \} \cup \tilde{S}_1 \wedge
		S_2 = \{(\bm{a}, c ) \} \cup \tilde{S}_2 \\
		
		\{(\bm{a}, b ) \} \cup (\tilde{S}_1 \pm S_2) & \text{if} \; S_1 = 	\{(\bm{a}, b ) \} \cup \tilde{S}_1 \wedge
		\{(\bm{a}, c ) \} \notin S_2 \;
...
\begin{equation}
	S_1 \cdot S_2 = 
	\begin{cases}
		\{(\bm{a} + \bm{\hat{a}}, b \cdot \hat{b} ) \} +  \{(\bm{a}, b ) \} \cdot \tilde{S}_2 + \tilde{S}_1 \cdot S_2 & \text{if} \; S_1 = \{(\bm{a}, b ) \} \cup \tilde{S}_1 \wedge
		S_2 = \{(\bm{\hat{a}}, \hat{b} ) \} \cup \tilde{S}_2 \\
