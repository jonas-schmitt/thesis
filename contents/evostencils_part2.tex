...
In order to estimate an upper bound for the achievable speedup, we can, therefore, compute the limit of $S(n)$ with respect to $n$ for a given value of $p$.
...
\begin{enumerate}
	...
	\item Elitist selection of the new population
\end{enumerate}
First of all, note that the amount of time required for the evaluation of each individual depends on the difficulty and size of the problem.
In contrast, the complexity of the other three steps is a function of the population size.
However, as a first step within each evaluation, we always have to invoke the ExaStencils compiler to generate a parallel C++ implementation.
As this step mainly depends on the number of expressions contained in each derivation tree, we can easily obtain a lower bound by considering common multigrid cycles.
